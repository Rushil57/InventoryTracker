    ...
    public class EquipmentRepository : IEquipmentRepository
    {
        ...
        public List<EquipmentTemplate> GetEquipmentTemplates(string equipmentType)
        {
            ...
            try
            {
                ...
                {
                    query = "SELECT Distinct [Equipment_Type] ,[Prop_Name] FROM [Equipment_Template]";
                }
                else
                {
        ...
        public bool SaveEquipmentHDR(EquipmentHeader equipmentHDR, List<EquipmentDetail> equipmentDtl, string EntityDtlID)
        {
            var connection = CommonDatabaseOperationHelper.CreateConnection();
            try
            {
                        ...
                        foreach (var ed in equipmentDtl)
                        {
                            ...
                            else
                            {
                                ...
                                else
                                {
                                    ...
                                    if (entDtlID > 0)
                                    {
                                        isRecordOfEndRange = isRecordOfEndRange + "and [Equip_Dtl_ID] NOT IN(" + entDtlID + ")";
        ...
        public DataTable ExportEquipmentEntityAssign(string startDate, string searchString, string columns)
        {
        ...
        public bool UpdateInsertEQUENTDateRangeASS(string startDate, List<string> columnHeader, List<string> values, int operation, out string totalNewAssigned, out int totalRemoved, out string invalidUnitID, out string totalNewUpdated)
        {
            var connection = CommonDatabaseOperationHelper.CreateConnection();
            var date = Convert.ToDateTime(startDate).ToString("MM/d/yyyy").Replace("-", "/");
            try
                    ...
                    for (int i = firstIndexOfUnitID; i < values.Count; i = i + 3)
                    {
                        if (!string.IsNullOrEmpty(values[i]) && !string.IsNullOrEmpty(values[i + 1]) && !string.IsNullOrEmpty(values[i + 2]) && (Convert.ToDateTime(values[i + 1]) <= Convert.ToDateTime(values[i + 2])))
                        {
                            ...
                            else
                            {
                                var sDate = Convert.ToDateTime(values[i + 1]).ToString("MM/d/yyyy").Replace("-", "/");
                                var eDate = Convert.ToDateTime(values[i + 2]).ToString("MM/d/yyyy").Replace("-", "/");
                                query += " IF (select count(*) from EQUIPMENT_ENTITY_ASSIGNMENT where ENT_ID = " + values[0] + " and Equip_ID = (select top 1 EQUIP_ID from EQUIPMENT_HDR where UNIT_ID = '" + values[i].Replace("'", "''") + "')   and ('" + date + "' between Start_Date and End_Date))  >= 1 BEGIN WITH CTE AS ( SELECT TOP 1 * FROM EQUIPMENT_ENTITY_ASSIGNMENT where ENT_ID = " + values[0] + " and Equip_ID = (select top 1 EQUIP_ID from EQUIPMENT_HDR where UNIT_ID = '" + values[i].Replace("'", "''") + "'   and ('" + date + "' between Start_Date and End_Date)) ORDER BY EQUIP_ENT_ID DESC) Update CTE SET START_DATE = '" + sDate + "', END_DATE ='" + eDate + "'; SET @equipIDList += CONVERT(varchar(100), (select top 1 EQUIP_ID from EQUIPMENT_HDR where UNIT_ID = '" + values[i].Replace("'", "''") + "')) + ','; END ";
                            }
                        }
                    }
                    var queryForUpdate = "select UNIT_ID  from (SELECT * from [dbo].[EQUIPMENT_ENTITY_ASSIGNMENT] where ENT_ID = " + values[0] + "  and ('" + date + "' between Start_Date and End_Date)) as eea left join EQUIPMENT_HDR as eh on eh.EQUIP_ID = eea.EQUIP_ID";
        ...
        public List<MapDetail> GetEquipmentNumericProp()
        {
            ...
            try
            {
                ...
                query = "SELECT distinct [Equip_Dtl_ID] ,ed.[Equip_ID] ,ed.[Equip_Temp_ID] ,et.Prop_Name ,[Eq_Value] ,eea.[Start_Date] ,eea.[End_Date], et.Datatype,et.Equipment_Type as EQUIP_TYPE,enh.ENT_TYPE,enh.ENT_ID,LOG( CONVERT(DECIMAL(18,2),Eq_Value)) as logEqValue FROM [dbo].[Equipment_Dtl] as ed join [dbo].[Equipment_Template] as et on ed.Equip_Temp_ID = et.Equip_Temp_ID join EQUIPMENT_ENTITY_ASSIGNMENT as eea on eea.EQUIP_ID = ed.Equip_ID join ENTITY_HDR enh on enh.ENT_ID = eea.ENT_ID where ed.Eq_Value != '' and ed.Eq_Value is not null  and Prop_Name != 'latitude' and Prop_Name != 'longitude' and (Datatype = 'int' or Datatype = 'decimal')";
        ...
        public List<MapDetail> GetEquipmentNullNumericProp()
        {
            ...
            try
            {
                ...
                query = "SELECT distinct [Equip_Dtl_ID] ,ed.[Equip_ID] ,ed.[Equip_Temp_ID] ,et.Prop_Name ,[Eq_Value] ,eea.[Start_Date] ,eea.[End_Date], et.Datatype,et.Equipment_Type as EQUIP_TYPE,enh.ENT_TYPE,enh.ENT_ID FROM [dbo].[Equipment_Dtl] as ed join [dbo].[Equipment_Template] as et on ed.Equip_Temp_ID = et.Equip_Temp_ID join EQUIPMENT_ENTITY_ASSIGNMENT as eea on eea.EQUIP_ID = ed.Equip_ID join ENTITY_HDR enh on enh.ENT_ID = eea.ENT_ID where (ed.Eq_Value = '' or ed.Eq_Value is null) and Prop_Name != 'latitude' and Prop_Name != 'longitude' and (Datatype = 'int' or Datatype = 'decimal')";
                equipmentTemplates = connection.Query<MapDetail>(query).ToList();
            }
            catch (Exception e)
            {
        ...
        public List<EquipmentTemplate> GetEquipmentTemplatesForMap()
        {
            List<EquipmentTemplate> equipmentTemplates = new List<EquipmentTemplate>();
            var connection = CommonDatabaseOperationHelper.CreateConnection();
            try
            {
                connection.Open();
                string query = string.Empty;
                query = "SELECT Distinct Equip_Temp_ID,[Equipment_Type] ,[Prop_Name],[Datatype] FROM [Equipment_Template]  where Datatype = 'int' or Datatype = 'Decimal'";
                equipmentTemplates = connection.Query<EquipmentTemplate>(query).ToList();
            }
            catch (Exception e)
            {
